...
regions = map(lambda r: r['RegionName'], session.client('ec2').describe_regions()['Regions'])
s3clients = {region: session.client('s3', region_name=region) for region in regions}
regions = sorted(filter(lambda r: r != origin_region, regions))
...
def create_bucket_if_not_exists(bucket_name, region):
	...
	if region != 'us-east-1':
	    get_s3(region).create_bucket(Bucket=bucket_name, CreateBucketConfiguration={'LocationConstraint': region})
	else:
	...
	get_s3(region).put_bucket_versioning(Bucket=bucket_name, VersioningConfiguration={'Status': 'Enabled'})
    else:
...
def destroy_bucket(bucket_name, region):
	...
	s3object.delete()
    sys.stderr.write('INFO: deleting all object versions from bucket %s\n' % bucket_name)
...
class ReplicationSetup(object):
	...
	self.role_name = 's3-%s-%s-replication' % (self.bucket_name_prefix, self.source_region)
	self.role_arn = 'arn:aws:iam::%s:role/%s' % (self.account_id, self.role_name)
	self.policy_name = self.role_name
	self.policy_arn = 'arn:aws:iam::%s:policy/%s' % (self.account_id, self.policy_name)
	...
	return {
	    ...
	    "Statement": [
		{
		    ...
		    "Resource":[
			"arn:aws:s3:::%s" % self.source_bucket
		    ]
		},
		{
		    "Effect": "Allow",
    ...
    def replication_configuration(self):
	    ...
	    'Rules': [
		{
		    'ID': '%s-%s-to-%s' % (self.bucket_name_prefix, self.source_region, self.target_region),
		    'Prefix': '',
		    'Status': 'Enabled',
		    'Destination': {
    ...
    def destroy_role(self):
		...
		for policy in role.attached_policies.all():
			sys.stderr.write('INFO: detaching policy %s role %s\n' % (policy.arn, self.role_name))
	...
	if current_role is None:
	    ...
	    iam.create_role(RoleName=self.role_name, AssumeRolePolicyDocument=json.dumps(role),
	...
	try:
	    ...
	    response = iam.get_policy_version(PolicyArn=self.policy_arn, VersionId=response['Policy']['DefaultVersionId'])
	...
	if current_policy is None:
	    response = iam.create_policy(PolicyName=self.policy_name, PolicyDocument=policy_document,
		...
		response = iam.create_policy_version(PolicyArn=self.policy_arn, SetAsDefault=True, PolicyDocument=policy_document)
	    else:
		sys.stderr.write('INFO: policy "%s" already exists.\n' % self.policy_name)

	sys.stderr.write('INFO: attaching policy %s to role "%s".\n' % (self.policy_name, self.role_name))
	response = iam.attach_role_policy(RoleName=self.policy_name, PolicyArn=self.policy_arn)
	...
	try:
	    response = get_s3(self.source_region).get_bucket_replication(Bucket=self.source_bucket)
	...
	if current_configuration is None:
	    ...
	    response = get_s3(self.source_region).put_bucket_replication(Bucket=self.source_bucket, ReplicationConfiguration=configuration)
	else:
