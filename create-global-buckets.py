...
parser.add_argument("--bucket-name", "-b", required=True, dest="global_bucket_name",
                    help="to create in all regions", metavar="STRING")
...
def create_iam_role():
    ...
    if current_role is None:
        ...
        iam.create_role(RoleName=role_name, AssumeRolePolicyDocument=json.dumps(role),
                        Description='s3 replication role for bucket %s' % global_bucket_name)
    else:
...
def create_and_attach_policy():
    ...
    if current_policy is None:
        response = iam.create_policy(PolicyName=policy_name, PolicyDocument=policy_document,
                                     Description='s3 replication policy for bucket %s' % global_bucket_name)
    else:
